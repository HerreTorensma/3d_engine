...
// 	if (ROTATING_C & entities[i].flags) {
// 		rotating_system();
// 	}
// }
...
void player_controller_system(res_pack_t *res_pack, grid_t *grid, SDL_Window *window, entity_t *player_entnt) {
	transform_c *transform = &player_entnt->transform;
	player_controller_c *controller = &player_entnt->player_controller;
	player_collider_c *collider = &player_entnt->player_collider;
	camera_c *camera = &player_entnt->camera;
...
int main(int argc, char *argv[]) {
	...
	// Make player
	// entity_t player_ent = ecs_new(&ecs);
	// {
	// 	transform_c *transform = ecs_set(&ecs, player_ent, TRANSFORM_C);
	...
	// 	// Or maybe not idk
	// 	camera_c *camera = ecs_set(&ecs, player_ent, CAMERA_C);
	// 	glm_vec3_copy((vec3){0.0f, 0.0f, -1.0f}, camera->front);
	// 	glm_vec3_copy((vec3){0.0f, 1.0f, 0.0f}, camera->up);
	// 	camera->yaw = -90.0f;

	// 	player_controller_c *controller = ecs_set(&ecs, player_ent, PLAYER_CONTROLLER_C);
	// 	controller->walk_speed = 0.05f;
	// 	controller->crouch_speed = 0.005f;
	// 	controller->jump_height = 0.08f;

	// 	player_collider_c *collider = ecs_set(&ecs, player_ent, PLAYER_COLLIDER_C);
	// 	collider->box = (box_t){
	// 		.min_x = -0.15f,
	// 		.max_x = 0.15f,
	...
	while (running) {
		...
		if (edit_mode) {
			...
			// transform_c *transform = ecs_get(&ecs, player_ent, TRANSFORM_C);
			// camera_t *camera = ecs_get(&ecs, player_ent, CAMERA_C);
